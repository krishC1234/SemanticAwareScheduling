#!/usr/bin/env python3
"""DenseNet121 - batch=64, large params (~8M original)"""
...
class DenseLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dropout > 0: out = F.dropout(out, p=self.dropout, training=self.training)
...
class DenseNet(nn.Module):
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64, bn_size=4, dropout=0.0, num_classes=1000):
        super().__init__()
...
def main():
    ...
    if rank == 0:
        ...
        print(f"densenet121_64_large | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
