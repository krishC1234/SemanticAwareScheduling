#!/usr/bin/env python3
"""EdgeCNN (Dynamic Graph CNN) - batch=32, small params (~0.5M)"""
...
class EdgeCNN(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(2, 1).contiguous()
        x1, x2 = self.conv1(x), self.conv2(self.conv1(x))
...
def main():
    ...
    if rank == 0:
        ...
        print(f"edgecnn_32_small | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
