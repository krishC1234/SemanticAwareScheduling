#!/usr/bin/env python3
"""BigBird - batch=2, ~5M params (Sparse Attention)"""
...
NUM_SAMPLES = 2000
VOCAB_SIZE = 30000
SEQ_LEN = 512  # BigBird handles longer sequences
ATTN_HEADS = 8
BLOCK_SIZE = 64  # For blocked sparse attention
NUM_GLOBAL_TOKENS = 2  # CLS-like global tokens
NUM_RANDOM_BLOCKS = 3  # Random attention blocks
...
class BigBirdAttention(nn.Module):
    """Simplified BigBird sparse attention with global + sliding window + random"""
    def __init__(self, hidden, heads, block_size=64, num_global=2, dropout=0.1):
        super().__init__()
        self.h = heads
    ...
    def forward(self, x, mask=None):
        ...
        # Full attention for simplicity in benchmark (sparse pattern adds complexity)
        # Real BigBird uses blocked sparse attention - this captures compute profile
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_k)
        
        # Create sparse mask: global tokens + sliding window
        if mask is None:
            mask = torch.ones(seq_len, seq_len, device=x.device)
            # Global tokens (first num_global) attend to all
            mask[:self.num_global, :] = 1
            mask[:, :self.num_global] = 1
            # Sliding window (3 blocks = block_size * 3)
            window = self.block_size * 3
...
class BigBirdBlock(nn.Module):
    def __init__(self, hidden, heads, block_size, num_global, dropout=0.1):
        super().__init__()
        self.attn = BigBirdAttention(hidden, heads, block_size, num_global, dropout)
...
class BigBird(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, seq_len, block_size, num_global):
        super().__init__()
        self.tok_emb = nn.Embedding(vocab_size, hidden)
        self.pos_emb = nn.Embedding(seq_len, hidden)
        self.blocks = nn.ModuleList([BigBirdBlock(hidden, heads, block_size, num_global) for _ in range(n_layers)])
...
class BigBirdForMLM(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, seq_len, block_size, num_global):
        super().__init__()
        self.bigbird = BigBird(vocab_size, hidden, n_layers, heads, seq_len, block_size, num_global)
        self.mlm = nn.Linear(hidden, vocab_size)
    def forward(self, x):
        return self.mlm(self.bigbird(x))
...
def main():
    ...
    model = BigBirdForMLM(VOCAB_SIZE, HIDDEN, N_LAYERS, ATTN_HEADS, SEQ_LEN, BLOCK_SIZE, NUM_GLOBAL_TOKENS).to(dev)
    pc = count_params(model)
    if rank == 0:
        print("###FEATURES###"); print(json.dumps({"model_type":"sparse_transformer","batch_size":BATCH_SIZE,"param_count":pc})); print("###END_FEATURES###")
        print(f"bigbird_2_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
