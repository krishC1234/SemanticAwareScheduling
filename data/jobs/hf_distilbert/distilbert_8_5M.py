#!/usr/bin/env python3
"""DistilBERT - batch=8, ~5M params (distilled BERT, no segment embeddings)"""
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        x = x + self.drop1(self.attn(x, mask))
        x = self.norm1(x)
        x = x + self.drop2(self.ff(x))
        return self.norm2(x)

class DistilBERT(nn.Module):
    """DistilBERT: No segment embeddings, fewer layers"""
...
class DistilBERTForMLM(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, seq_len):
        super().__init__()
        self.distilbert = DistilBERT(vocab_size, hidden, n_layers, heads, seq_len)
        self.mlm = nn.Linear(hidden, vocab_size)
    def forward(self, x):
        return self.mlm(self.distilbert(x))
...
def main():
    ...
    model = DistilBERTForMLM(VOCAB_SIZE, HIDDEN, N_LAYERS, ATTN_HEADS, SEQ_LEN).to(dev)
    pc = count_params(model)
    if rank == 0:
        print("###FEATURES###"); print(json.dumps({"model_type":"transformer","batch_size":BATCH_SIZE,"param_count":pc})); print("###END_FEATURES###")
        print(f"distilbert_8_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
