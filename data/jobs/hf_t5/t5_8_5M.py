#!/usr/bin/env python3
"""T5 - batch=8, ~5M params (Encoder-Decoder, Span Corruption)"""
...
BATCH_SIZE = 8
HIDDEN = 256
N_LAYERS = 4  # Per encoder/decoder
...
class T5LayerNorm(nn.Module):
    """T5-style layer norm (no bias, variance only)"""
...
class T5Attention(nn.Module):
    ...
    def __init__(self, hidden, heads, is_decoder=False, has_cross_attn=False, dropout=0.1):
        ...
        # Relative position bias (simplified - shared across heads)
    ...
    def forward(self, x, enc_out=None, mask=None):
        ...
        # Add relative position bias (for self-attention only)
        if enc_out is None:
            positions = torch.arange(seq_len, device=x.device)
            rel_pos = positions.unsqueeze(1) - positions.unsqueeze(0)
            rel_pos = rel_pos.clamp(-16, 15) + 16  # Map to [0, 31]
...
class T5FeedForward(nn.Module):
    """T5 feed-forward with gated GELU"""
    def __init__(self, hidden, ff_dim, dropout=0.1):
        super().__init__()
        self.wi_0 = nn.Linear(hidden, ff_dim, bias=False)  # Gate
        self.wi_1 = nn.Linear(hidden, ff_dim, bias=False)  # Up
        self.wo = nn.Linear(ff_dim, hidden, bias=False)    # Down
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x):
        gate = torch.relu(self.wi_0(x))  # T5 uses ReLU for gating
        hidden = self.wi_1(x)
        x = gate * hidden
...
class T5(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, ff_dim):
        ...
        # No position embeddings - uses relative position bias
...
class SyntheticT5Dataset(Dataset):
    """Span corruption dataset (simplified)"""
    def __init__(self, size, vocab_size, seq_len):
        self.size, self.vocab_size, self.seq_len = size, vocab_size, seq_len
        self.sentinel_start = vocab_size - 100  # Reserve last 100 tokens as sentinels
        
    def __len__(self): return self.size
    
    def __getitem__(self, i):
        # Encoder input: original with spans replaced by sentinels
        enc_ids = torch.randint(1, self.sentinel_start, (self.seq_len,))
        # Decoder input/target: sentinel + span tokens
...
def main():
    ...
    if rank == 0:
        ...
        print(f"t5_8_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
