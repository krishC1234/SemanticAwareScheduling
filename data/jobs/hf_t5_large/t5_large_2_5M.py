#!/usr/bin/env python3
"""T5-Large - batch=2, ~5M params (Encoder-Decoder, Span Corruption)"""
...
class T5LayerNorm(nn.Module):
    """T5-style layer norm (RMSNorm - no bias, variance only)"""
...
def main():
    ...
    model = T5Large(VOCAB_SIZE, HIDDEN, N_LAYERS, ATTN_HEADS, ff_dim).to(dev)
    pc = count_params(model)
    if rank == 0:
        print("###FEATURES###"); print(json.dumps({"model_type":"encoder_decoder","batch_size":BATCH_SIZE,"param_count":pc})); print("###END_FEATURES###")
        print(f"t5_large_2_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
