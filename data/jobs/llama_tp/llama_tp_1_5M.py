#!/usr/bin/env python3
"""LLaMA TP (Tensor Parallel) - batch=1, ~5M params (DTensor-style separate QKV)"""
...
class LlamaAttentionTP(nn.Module):
    """LLaMA Attention with separate Q, K, V projections for tensor parallelism"""
    def __init__(self, hidden, heads):
        super().__init__()
        self.h = heads
        self.d_k = hidden // heads
        # Separate projections for TP compatibility (ColwiseParallel)
...
class LlamaMLPTP(nn.Module):
    """LLaMA MLP with SwiGLU for tensor parallelism (fc1, fc2 ColwiseParallel, c_proj RowwiseParallel)"""
    def __init__(self, hidden, ff_dim):
        super().__init__()
        # Gate and up projections (ColwiseParallel)
        self.c_fc1 = nn.Linear(hidden, ff_dim, bias=False)  # Gate (w1)
        self.c_fc2 = nn.Linear(hidden, ff_dim, bias=False)  # Up (w3)
        # Down projection (RowwiseParallel)
        self.c_proj = nn.Linear(ff_dim, hidden, bias=False)  # Down (w2)
...
class LlamaTP(nn.Module):
    """LLaMA model structured for tensor parallelism"""
...
def main():
    ...
    if rank == 0:
        ...
        print(f"llama_tp_1_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
