#!/usr/bin/env python3
"""nanoGPT - batch=1, ~5M params (Karpathy's minGPT/nanoGPT)"""
...
SEQ_LEN = 128
ATTN_HEADS = 8
DROPOUT = 0.0  # nanoGPT typically uses 0.0 for pretraining
...
class MLP(nn.Module):
    """nanoGPT MLP with GELU activation"""
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, lr, betas, device):
        """nanoGPT-style optimizer with weight decay only for weights, not biases/LayerNorm"""
        decay = set()
        no_decay = set()
        whitelist = (nn.Linear,)
...
def main():
    ...
    if rank == 0:
        ...
        print(f"nanogpt_1_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
    ...
    # nanoGPT-style optimizer
    optim = model.module.configure_optimizers(
        weight_decay=0.1, lr=6e-4, betas=(0.9, 0.95), device=dev
    )
