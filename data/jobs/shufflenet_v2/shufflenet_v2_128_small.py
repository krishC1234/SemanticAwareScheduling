#!/usr/bin/env python3
"""ShuffleNetV2 - batch=128 (original), small params (~1.4M with x0.5)"""
...
# === VARYING ===
BATCH_SIZE = 128
STAGES_OUT = [24, 48, 96, 192, 1024]  # x0.5 config
...
def channel_shuffle(x, groups):
    ...
    return x.view(bs, num_channels, h, w)
...
class ShuffleNetV2(nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, num_classes=1000):
        ...
        for repeats, out_channels in zip(stages_repeats, stages_out_channels[1:-1]):
            stage = []
            for j in range(repeats):
                stage.append(InvertedResidual(in_channels, out_channels, 2 if j == 0 else 1))
                in_channels = out_channels
            self.stages.append(nn.Sequential(*stage))
        self.conv5 = nn.Sequential(
            nn.Conv2d(in_channels, stages_out_channels[-1], 1, bias=False),
...
def main():
    ...
    if rank == 0:
        ...
        print(f"shufflenet_v2_128_small | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
