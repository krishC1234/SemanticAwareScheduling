#!/usr/bin/env python3
"""T5-Base - encoder-decoder seq2seq, batch=8, ~220M params

T5 (Text-to-Text Transfer Transformer) uses a full encoder-decoder
architecture with relative position bias instead of absolute position
embeddings. T5-Base: 12 encoder layers, 12 decoder layers, 768 hidden,
12 heads, 3072 FFN dim.
...
ENC_SEQ_LEN = 512
DEC_SEQ_LEN = 128
NUM_LAYERS = 12          # each for encoder and decoder
HIDDEN = 768
NUM_HEADS = 12
FFN_DIM = 3072
DROPOUT = 0.1
NUM_BUCKETS = 32         # relative position bias buckets
MAX_DISTANCE = 128
...
class T5RelativePositionBias(nn.Module):
    """Learned relative position bias (no absolute position embeddings)."""
    ...
    def forward(self, qlen, klen):
        ctx = self.bias.weight.device
        q_pos = torch.arange(qlen, device=ctx)[:, None]
        k_pos = torch.arange(klen, device=ctx)[None, :]
        buckets = self._relative_position_bucket(k_pos - q_pos)
        values = self.bias(buckets)  # (qlen, klen, heads)
        return values.permute(2, 0, 1).unsqueeze(0)  # (1, heads, qlen, klen)
...
class T5Attention(nn.Module):
    ...
    def forward(self, x, context=None, causal=False):
        ...
        def reshape(t, length): return t.view(B, length, NUM_HEADS, self.head_dim).transpose(1, 2)
        q = reshape(self.q(x), S)
        k = reshape(self.k(kv_input), T)
        v = reshape(self.v(kv_input), T)
...
class T5LayerNorm(nn.Module):
    """T5 uses RMSNorm (no bias, no mean subtraction)."""
    def __init__(self):
        super().__init__()
...
class T5EncoderBlock(nn.Module):
    def __init__(self):
        super().__init__()
        self.norm1 = T5LayerNorm()
        self.attn = T5Attention(bidirectional=True)
        self.norm2 = T5LayerNorm()
        self.ffn = T5FFN()
        self.drop = nn.Dropout(DROPOUT)
...
def main():
    ...
    if rank == 0:
        ...
        print("###END_FEATURES###")
        print(f"hf_t5_base | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
