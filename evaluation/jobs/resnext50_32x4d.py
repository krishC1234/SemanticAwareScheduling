#!/usr/bin/env python3
"""ResNeXt-50 (32×4d) - batch=8, ~25M params

ResNeXt extends ResNet by replacing standard 3×3 convolutions with
grouped convolutions (cardinality=32, bottleneck_width=4d). This
increases representational capacity without significantly increasing
compute vs a ResNet-50.

Reference: Xie et al., "Aggregated Residual Transformations for
...
class BottleneckX(nn.Module):
    """ResNeXt bottleneck: 1×1 → 3×3 (grouped) → 1×1.
    Groups = cardinality, group width = base_width * (planes/64)."""
...
class ResNeXt50(nn.Module):
    """ResNeXt-50 (32×4d): ~25M trainable parameters.

    Layer config: [3, 4, 6, 3] blocks with planes [64, 128, 256, 512]."""
    ...
    def __init__(self, num_classes=1000):
        ...
        self.fc = nn.Linear(512 * BottleneckX.expansion, num_classes)
    ...
    def _make_layer(self, planes, blocks, stride):
        downsample = None
        out_ch = planes * BottleneckX.expansion
        if stride != 1 or self.inplanes != out_ch:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, out_ch, 1, stride=stride, bias=False),
                nn.BatchNorm2d(out_ch),
            )
        layers = [BottleneckX(self.inplanes, planes, stride, downsample)]
        self.inplanes = out_ch
        for _ in range(1, blocks):
            layers.append(BottleneckX(self.inplanes, planes))
...
def main():
    ...
    model = ResNeXt50(NUM_CLASSES).to(dev)
    pc = count_params(model)
    if rank == 0:
        print("###FEATURES###")
        print(json.dumps({"model_type": "cnn", "batch_size": BATCH_SIZE, "param_count": pc}))
        print("###END_FEATURES###")
        print(f"resnext50_32x4d | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
