#!/usr/bin/env python3
"""CycleGAN - batch=2, large params (~20M with ngf=ndf=64)"""
...
def main():
    ...
    if rank == 0:
        ...
        print(f"cyclegan_2_large | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
    ...
    for ep in range(EPOCHS):
        ...
        for real_A, real_B in loader:
            ...
            b = real_A.size(0)
            valid = torch.ones(b, 1, 30, 30, device=dev)
            fake_t = torch.zeros(b, 1, 30, 30, device=dev)
            opt_G.zero_grad()
