#!/usr/bin/env python3
"""GPT-2 - batch=4, ~5M params (Causal LM, decoder-only)"""
...
class CausalSelfAttention(nn.Module):
    def __init__(self, hidden, heads, seq_len, dropout=0.1):
        ...
        self.c_attn = nn.Linear(hidden, 3 * hidden)  # Q, K, V combined
...
class GPT2(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, seq_len):
        super().__init__()
        self.wte = nn.Embedding(vocab_size, hidden)  # Token embeddings
        self.wpe = nn.Embedding(seq_len, hidden)     # Position embeddings
...
def main():
    ...
    if rank == 0:
        ...
        print(f"gpt2_4_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
