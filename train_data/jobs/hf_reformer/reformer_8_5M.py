#!/usr/bin/env python3
"""Reformer - batch=8, ~5M params (LSH Attention + Reversible Layers)"""
...
NUM_SAMPLES = 2000
VOCAB_SIZE = 30000
SEQ_LEN = 256  # Reformer designed for longer sequences
ATTN_HEADS = 8
NUM_HASHES = 2  # Number of hash rounds for LSH
NUM_BUCKETS = 32  # Number of buckets for LSH
...
class LSHAttention(nn.Module):
    """Simplified LSH (Locality-Sensitive Hashing) Attention"""
    ...
    def forward(self, x):
        ...
        # In real Reformer, Q=K for self-attention (shared QK)
        qk = self.q_lin(x)  # Shared Q and K
        v = self.v_lin(x)
        
        qk = qk.view(bs, seq_len, self.h, self.d_k).transpose(1, 2)
        v = v.view(bs, seq_len, self.h, self.d_k).transpose(1, 2)
        
        # Simplified: use chunked attention instead of full LSH for benchmark
        # This approximates the compute pattern of LSH attention
        chunk_size = max(1, seq_len // self.num_buckets)
        
        # Standard attention within chunks (approximates LSH bucketing)
...
class ChunkedFeedForward(nn.Module):
    """Chunked feed-forward to save memory"""
    ...
    def forward(self, x):
        # Process in chunks to save memory (key Reformer feature)
...
class ReversibleBlock(nn.Module):
    """Simplified reversible block (without actual reversibility for benchmark)"""
    ...
    def forward(self, x):
        # In real Reformer, this would be reversible (Y1 = X1 + Attn(X2), Y2 = X2 + FF(Y1))
        # Simplified to standard residual for benchmark
...
def main():
    ...
    if rank == 0:
        ...
        print(f"reformer_8_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
