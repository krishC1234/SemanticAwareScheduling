#!/usr/bin/env python3
"""RoBERTa - batch=4, ~5M params (MLM only, no NSP, dynamic masking)"""
...
class RoBERTa(nn.Module):
    """RoBERTa: No segment embeddings, MLM only"""
    def __init__(self, vocab_size, hidden, n_layers, heads, seq_len):
        super().__init__()
        self.tok_emb = nn.Embedding(vocab_size, hidden, padding_idx=0)
        self.pos_emb = nn.Embedding(seq_len + 2, hidden)  # +2 for special tokens
        # No segment embeddings (key difference from BERT)
    ...
    def forward(self, x):
        ...
        # Position IDs start from padding_idx + 1 (RoBERTa convention)
...
class RoBERTaForMLM(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, seq_len):
        ...
        # MLM head with layer norm (RoBERTa style)
...
class SyntheticMLMDataset(Dataset):
    """Dynamic masking (RoBERTa style) - masks are generated on-the-fly"""
    def __init__(self, size, vocab_size, seq_len, mask_prob=0.15):
        ...
        self.mask_token_id = 1  # [MASK] token
        
    def __len__(self): return self.size
    
    def __getitem__(self, i):
        ids = torch.randint(2, self.vocab_size, (self.seq_len,))  # Start from 2 (0=pad, 1=mask)
        labels = ids.clone()
        
        # Dynamic masking: randomly select positions to mask
...
def main():
    ...
    if rank == 0:
        ...
        print(f"roberta_4_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
