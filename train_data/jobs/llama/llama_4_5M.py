#!/usr/bin/env python3
"""LLaMA - batch=4, ~5M params (RoPE, SwiGLU, RMSNorm)"""
...
SEQ_LEN = 128
ATTN_HEADS = 8
FF_MULT = 4  # FFN hidden = hidden * ff_mult * 2/3 for SwiGLU
...
class LlamaAttention(nn.Module):
    ...
    def forward(self, x, freqs_cis, mask=None):
        ...
        # Transpose for attention: (bs, heads, seq, d_k)
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
...
class LlamaFeedForward(nn.Module):
    """SwiGLU FFN: out = (Swish(x @ W1) * (x @ W3)) @ W2"""
    def __init__(self, hidden, ff_dim, dropout=0.1):
        super().__init__()
        # SwiGLU uses 2/3 * 4 * hidden for intermediate
        self.w1 = nn.Linear(hidden, ff_dim, bias=False)  # Gate
        self.w2 = nn.Linear(ff_dim, hidden, bias=False)  # Down
        self.w3 = nn.Linear(hidden, ff_dim, bias=False)  # Up
        self.dropout = nn.Dropout(dropout)
...
class LlamaBlock(nn.Module):
    def __init__(self, hidden, heads, ff_dim, dropout=0.1):
        super().__init__()
        self.attention = LlamaAttention(hidden, heads, dropout)
        self.feed_forward = LlamaFeedForward(hidden, ff_dim, dropout)
...
class Llama(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, ff_dim, seq_len):
        ...
        self.layers = nn.ModuleList([LlamaBlock(hidden, heads, ff_dim) for _ in range(n_layers)])
...
def main():
    ...
    # SwiGLU intermediate size: 2/3 * 4 * hidden (rounded)
    ff_dim = int(2 * HIDDEN * FF_MULT / 3)
    ff_dim = ((ff_dim + 63) // 64) * 64  # Round to multiple of 64
    
    model = Llama(VOCAB_SIZE, HIDDEN, N_LAYERS, ATTN_HEADS, ff_dim, SEQ_LEN).to(dev)
    pc = count_params(model)
    if rank == 0:
        print("###FEATURES###"); print(json.dumps({"model_type":"causal_lm","batch_size":BATCH_SIZE,"param_count":pc})); print("###END_FEATURES###")
        print(f"llama_4_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
