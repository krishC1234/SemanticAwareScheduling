#!/usr/bin/env python3
"""LLaMA-2 7B 16h - batch=1, ~5M params (RoPE, SwiGLU, RMSNorm, extended context)"""
...
NUM_SAMPLES = 2000
VOCAB_SIZE = 32000
SEQ_LEN = 256  # LLaMA-2 supports longer context (4096 in full model)
ATTN_HEADS = 16  # 16 heads as per model name
FF_MULT = 4

class RMSNorm(nn.Module):
    """LLaMA-2 RMSNorm"""
    def __init__(self, dim, eps=1e-5):  # LLaMA-2 uses 1e-5
        super().__init__()
        self.eps = eps
...
class Llama2Attention(nn.Module):
    def __init__(self, hidden, heads, dropout=0.0):  # LLaMA-2 uses no dropout
        super().__init__()
        self.h = heads
...
class Llama2(nn.Module):
    def __init__(self, vocab_size, hidden, n_layers, heads, ff_dim, seq_len):
        ...
        # Note: LLaMA-2 does NOT tie embeddings (unlike LLaMA-1)
...
def main():
    ...
    ff_dim = ((ff_dim + 255) // 256) * 256  # Multiple of 256 for LLaMA-2
    ...
    if rank == 0:
        ...
        print(f"llama2_1_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
