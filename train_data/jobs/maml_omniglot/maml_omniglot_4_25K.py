#!/usr/bin/env python3
"""MAML Omniglot (Meta-Learning) - task_num=4, ~25K params"""
...
from torch.func import functional_call, grad, vmap, stack_module_state
import warnings
warnings.filterwarnings("ignore")

TASK_NUM = 4  # Number of tasks per meta-batch
N_WAY = 5     # N-way classification
K_SHOT = 1    # K-shot (support set size per class)
Q_QUERY = 15  # Query set size per class
N_INNER_ITER = 5
HIDDEN_CH = 32
IMG_SIZE = 28
EPOCHS = 3
NUM_META_BATCHES = 300
LR_META = 1e-3
LR_INNER = 0.1
...
class MAMLNet(nn.Module):
    """4-layer ConvNet for few-shot classification"""
    def __init__(self):
        super().__init__()
...
def compute_loss(params, buffers, model, x, y):
    """Compute loss for functional model call"""
...
class OmniglotTaskDataset(Dataset):
    """Synthetic few-shot tasks (Omniglot-like)"""
    ...
    def __getitem__(self, i):
        ...
        # Shuffle
        perm_spt = torch.randperm(spt_size)
        perm_qry = torch.randperm(qry_size)
...
def main():
    ...
    if rank == 0:
        ...
        print(f"maml_omniglot_4_25K | GPUs:{ws} | Tasks:{TASK_NUM} | Params:{pc:,}")
    
    # Get functional representation
    ...
    for ep in range(EPOCHS):
        ...
        for x_spt, y_spt, x_qry, y_qry in loader:
            ...
            for t in range(TASK_NUM):
                loss, acc = meta_loss_for_task(
                    model, params, buffers,
                    x_spt[t], y_spt[t], x_qry[t], y_qry[t]
                )
