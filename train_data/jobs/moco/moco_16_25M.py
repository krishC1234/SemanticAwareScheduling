#!/usr/bin/env python3
"""MoCo (Momentum Contrast) - batch=16, ~25M params (ResNet-18)"""
...
BATCH_SIZE = 16
MOCO_DIM = 128      # Output dimension
MOCO_K = 4096       # Queue size
MOCO_M = 0.999      # Momentum coefficient
...
class MoCoDataset(Dataset):
    """Synthetic two-view dataset for MoCo"""
    def __init__(self, sz):
        self.sz = sz
    def __len__(self): return self.sz
    def __getitem__(self, i):
        # Two augmented views of same image
        im_q = torch.rand(3, 224, 224)
        im_k = torch.rand(3, 224, 224)
        return im_q, im_k
...
def main():
    ...
    if rank == 0:
        ...
        print(f"moco_16_25M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
