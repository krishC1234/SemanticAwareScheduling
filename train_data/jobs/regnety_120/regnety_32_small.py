#!/usr/bin/env python3
"""RegNetY-120 - batch=32, small params (~13M)"""
...
# === VARYING ===
BATCH_SIZE = 32
WIDTH_MULT = 0.5  # Scale down channel counts
...
class YBlock(nn.Module):
    """RegNetY bottleneck block with SE."""
    def __init__(self, in_ch, out_ch, stride=1, group_width=24, bottleneck_ratio=1.0, se_ratio=0.25):
        ...
        mid_ch = groups * group_width  # Ensure divisibility
...
class RegNetY(nn.Module):
    def __init__(self, num_classes=1000, width_mult=1.0, dropout=0.0):
        ...
        # RegNetY-120 configuration: [depth, width, stride, group_width]
        # Based on the regnet design space with ~120 GFLOPs
        stage_config = [
            [2, 336, 2, 24],
            [5, 672, 2, 24],
            [11, 1344, 2, 24],
...
def main():
    ...
    if rank == 0:
        ...
        print(f"regnety_32_small | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
