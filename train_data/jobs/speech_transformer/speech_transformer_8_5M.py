#!/usr/bin/env python3
"""Speech Transformer - batch=8, ~5M params (ASR encoder-decoder)"""
...
BATCH_SIZE = 8
HIDDEN = 256
N_LAYERS = 4  # encoder and decoder layers

# === FIXED ===
EPOCHS = 3
NUM_SAMPLES = 2000
VOCAB_SIZE = 4233  # aishell character vocab
ATTN_HEADS = 8
...
class SpeechTransformer(nn.Module):
    ...
    def make_pad_mask(self, lengths, max_len):
        ...
        return mask.unsqueeze(1).unsqueeze(2)  # (bs, 1, 1, seq_len)
    ...
    def forward(self, padded_input, input_lengths, padded_target):
        """
        Args:
            padded_input: (batch, seq_len, input_dim) mel spectrogram
            input_lengths: (batch,) actual lengths
            padded_target: (batch, target_len) token indices
        """
        ...
        for layer in self.decoder_layers:
            y = layer(y, enc_out, tgt_mask, memory_mask)
...
class SyntheticSpeechDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Labels (shifted target for teacher forcing)
...
def main():
    ...
    model = SpeechTransformer(
        INPUT_DIM, VOCAB_SIZE, HIDDEN, N_LAYERS, ATTN_HEADS, ff_dim,
        MAX_SEQ_LEN, MAX_TARGET_LEN, DROPOUT
    ).to(dev)
    ...
    if rank == 0:
        ...
        print(f"speech_transformer_8_5M | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
    ...
    for ep in range(EPOCHS):
        ...
        for padded_input, input_lengths, padded_target, labels in loader:
            padded_input = padded_input.to(dev)
            input_lengths = input_lengths.to(dev)
            padded_target = padded_target.to(dev)
