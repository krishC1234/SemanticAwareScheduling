#!/usr/bin/env python3
"""Stable Diffusion UNet - batch=1, large params (~200M)"""
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        return self.to_out(out).permute(0, 2, 1).view(b, c, h, w)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context):
        return x + self.proj_out(self.attn(self.proj_in(self.norm(x)), context))
...
class DownBlock(nn.Module):
    ...
    def forward(self, x, t_emb, context):
        ...
        for res, attn in zip(self.res_blocks, self.attn_blocks):
            ...
            outputs.append(x)
        if self.downsample: x = self.downsample(x); outputs.append(x)
        return x, outputs
...
class UNet(nn.Module):
    def __init__(self, in_ch, out_ch, base_ch, ch_mult, num_res_blocks, attn_resolutions, context_dim, time_emb_dim):
        ...
        in_ch_curr, curr_res = base_ch, LATENT_SIZE
        ...
        for i, out_ch in enumerate(reversed(channels)):
            ...
            attn = curr_res in attn_resolutions
            self.up_blocks.append(UpBlock(in_ch, out_ch, out_ch, time_emb_dim, num_res_blocks, attn, context_dim, i < len(channels) - 1))
            if i < len(channels) - 1: curr_res *= 2
    ...
    def forward(self, x, t, context):
        ...
        x = self.conv_in(x)
        skips = []
        for block in self.down_blocks: x, skip_outputs = block(x, t_emb, context); skips.extend(skip_outputs)
        x = self.mid_block2(self.mid_attn(self.mid_block1(x, t_emb), context), t_emb)
        for block in self.up_blocks: x = block(x, skips, t_emb, context)
...
class SyntheticDiffusionDataset(Dataset):
    def __init__(self, size, latent_size, latent_ch, context_dim, seq_len=77):
        self.size, self.latent_size, self.latent_ch, self.context_dim, self.seq_len = size, latent_size, latent_ch, context_dim, seq_len
...
def main():
    ...
    model = UNet(LATENT_CHANNELS, LATENT_CHANNELS, BASE_CHANNELS, CHANNEL_MULT, NUM_RES_BLOCKS, ATTN_RESOLUTIONS, CONTEXT_DIM, TIME_EMB_DIM).to(dev)
    pc = count_params(model)
    if rank == 0:
        print("###FEATURES###"); print(json.dumps({"model_type":"diffusion_unet","batch_size":BATCH_SIZE,"param_count":pc})); print("###END_FEATURES###")
        print(f"sd_1_large | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
    ...
    for ep in range(EPOCHS):
        ...
        for latent, noise, t, context in loader:
            ...
            optim.zero_grad()
            loss = mse(model(noisy, t, context), noise)
