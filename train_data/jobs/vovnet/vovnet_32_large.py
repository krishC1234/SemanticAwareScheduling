#!/usr/bin/env python3
"""VoVNet (Variety of View Network) - batch=32, large params (~23M)"""
...
BATCH_SIZE = 32
BASE_CHANNELS = 64
STAGE_CHANNELS = [256, 320, 384, 448]  # VoVNet39a-like
LAYER_PER_BLOCK = 5
EPOCHS = 3
NUM_SAMPLES = 2000
IMG_SIZE = 224
...
class OSAModule(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, num_layers=5, reduction=True):
        super().__init__()
        self.layers = nn.ModuleList([conv_bn_relu(in_ch, mid_ch, 3, 1, 1)])
        for _ in range(num_layers - 1):
            self.layers.append(conv_bn_relu(mid_ch, mid_ch, 3, 1, 1))
        self.agg = conv_bn_relu(in_ch + mid_ch * num_layers, out_ch, 1, 1, 0)
        self.reduction = reduction
        if reduction: self.pool = nn.MaxPool2d(3, 2, 1)
    def forward(self, x):
        outputs = [x]
        for layer in self.layers: x = layer(x); outputs.append(x)
        x = self.agg(torch.cat(outputs, 1))
        return self.pool(x) if self.reduction else x

class VoVNet(nn.Module):
    def __init__(self, base_ch=BASE_CHANNELS, stage_ch=STAGE_CHANNELS, lpb=LAYER_PER_BLOCK):
        super().__init__()
        self.stem = nn.Sequential(conv_bn_relu(IN_CHANNELS, base_ch//2, 3, 2, 1), conv_bn_relu(base_ch//2, base_ch//2, 3, 1, 1), conv_bn_relu(base_ch//2, base_ch, 3, 1, 1), nn.MaxPool2d(3, 2, 1))
        self.stages = nn.ModuleList()
        ic = base_ch
        for i, oc in enumerate(stage_ch):
            self.stages.append(OSAModule(ic, oc//2, oc, lpb, i < len(stage_ch)-1)); ic = oc
...
def main():
    ...
    if rank == 0:
        ...
        print(f"vovnet_32_large | GPUs:{ws} | Batch:{BATCH_SIZE} | Params:{pc:,}")
    ...
    crit, opt = nn.CrossEntropyLoss(), torch.optim.SGD(model.parameters(), lr=LR, momentum=0.9, weight_decay=1e-4)
    ts, tsp = time.time(), 0
